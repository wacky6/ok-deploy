...
_install_crontab () {
  ...
  crontab -l | {
    cat ;
    echo "$RAND_MIN 5 * * * ( echo $CMD_PREFIX ; cd "${HOME_DIR}/my_zshrc" ; git fetch origin && git merge --ff-only origin/master ; cd "${HOME_DIR}/.oh-my-zsh" ; git fetch origin ; git reset --hard FETCH_HEAD ) "
  } | crontab -
